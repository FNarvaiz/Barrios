...
{
    [Migration(20200302104400), Tags("Default")]
    public class DefaultDB_20200302_104400_UpdateReservasStoreProcedure : Migration
    {
        public override void Up()
        {

            Create.Column("AmountToReserve").OnTable("[RESERVAS_RECURSOS]").AsInt16().WithDefaultValue(1);
            Create.Column("TenantLimitDate").OnTable("[Users]").AsDate().Nullable();
            Create.Column("Owner").OnTable("[Users]").AsBoolean().NotNullable().WithDefaultValue(true);

            Execute.Sql("UPDATE [RESERVAS_RECURSOS] SET  AmountToReserve=2 WHERE resolucion >0");

            Execute.Sql("UPDATE RR SET  desde=0, Hasta=B.CantDiasReservables from [RESERVAS_RECURSOS] RR inner join [Recursos-Barrios] RB on RR.id= RB.RecursoId inner join BARRIOS B on B.id=RB.BarrioId WHERE resolucion >0");
            ...
            query.Append("                    select @UserUnit=unit from Users where userid = @UserId");

            query.Append("                    If not @resourceId is null ");
            query.Append("                    Begin ");

            query.Append("                        SELECT  @AmountToReserve=RR.AmountToReserve,@turnDuration = RT.DURACION, ");
            ...
            query.Append("                          ON (R.ID_VECINO = US.UserId OR R.ID_VECINO_2 = US.UserId) AND US.Unit = @UserUnit "); 
            query.Append("                          inner join [Users-Barrios] UB");
            query.Append("                          ON UB.UserId = US.UserId AND UB.BarrioId = @BarrioId ");
            query.Append("                          WHERE FECHA_FIN > GETDATE() ");

            query.Append("                        If @count < @AmountToReserve or @admin=1 ");// REVISA CUANTAS RESERVAS TIENE REALIZADAS
            query.Append("                        BEGIN ");
